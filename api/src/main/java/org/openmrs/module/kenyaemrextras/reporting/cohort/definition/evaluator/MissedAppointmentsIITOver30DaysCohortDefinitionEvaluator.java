...
public class MissedAppointmentsIITOver30DaysCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		String qry = "select a.patient_id\n"
		        ...
		        + "                                                      f6.refill_tca as refill_tca,\n"
		        + "                                                      k.patient_id as refill_patient,\n"
		        + "                                                      min(k.visit_date) as refill_return_date\n"
		        + "                                                    --  least(ifnull(min(k.visit_date),min(f4.visit_date)),min(f4.visit_date)) as rtc_date\n"
		        ...
		        + "                                                                          mid(max(concat(f5.visit_date, f5.refill_date)), 11) as refill_tca,\n"
		        + "                                                                          k.patient_id as refill_patient,\n"
		        + "                                                                          k.refill_visit_date as refill_visit_date\n"
		        ...
		        + "                                                                            left join (select k.patient_id, max(k.visit_date) as refill_visit_date\n"
		        ...
		        + "                                                                      or f5.refill_date between date(:startDate) and date(:endDate)\n"
		        + "                                                           group by f5.patient_id) f6 on f4.patient_id = f6.patient_id\n"
		        + "                                                        left join kenyaemr_etl.etl_art_fast_track k on f4.patient_id = k.patient_id\n"
		        + "                                       where f4.visit_date > f6.app_date or k.visit_date > f6.app_date\n"
		        ...
		        + "                                             f7.refill_patient as refill_patient,\n"
		        + "                                             f7.refill_return_date as refill_return_date\n"
		        ...
		        + "                                                         max(k.visit_date)                                            as refill_return_date,\n"
		        ...
		        + "                                                                   left join kenyaemr_etl.etl_art_fast_track k on f7.patient_id = k.patient_id\n"
		        ...
		        + "                                and (f7.return_date = f0.visit_date or f7.refill_return_date = f0.visit_date) -- Is this accurate?\n"
		        ...
		        + "                             date(d.visit_date) >= date(:endDate))) a where a.days_missed > 30 or a.refill_days_missed > 30;";
