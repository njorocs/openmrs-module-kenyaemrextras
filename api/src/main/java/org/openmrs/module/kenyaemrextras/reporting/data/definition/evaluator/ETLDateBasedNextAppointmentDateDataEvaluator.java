...
public class ETLDateBasedNextAppointmentDateDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select a.patient_id,\n"
		        + "       ifnull(a.refill_appointment_date, least(ifnull(a.refill_appointment_date, a.next_appointment_date),\n"
		        + "                                               next_appointment_date)) as date_missed_appointment\n"
		        + "from (select r.patient_id,\n"
		        + "             r.app_visit,\n"
		        + "             r.return_date,\n"
		        + "             r.next_appointment_date as next_appointment_date,\n"
		        + "             r.next_refill_date      as refill_appointment_date,\n"
		        ...
		        + "                      max(k.visit_date)                                             as refill_visit_date,\n"
		        + "                      mid(max(concat(f4.visit_date, f4.next_appointment_date)), 11) as next_appointment_date,\n"
		        + "                      mid(max(concat(f4.visit_date, f4.refill_date)), 11)           as next_refill_date\n"
		        + "               from kenyaemr_etl.etl_patient_hiv_followup f4\n"
		        + "                        left join (select f5.patient_id, f5.visit_date, f5.next_appointment_date, f5.refill_date\n"
		        + "                                   from kenyaemr_etl.etl_patient_hiv_followup f5) f5\n"
		        + "                                  on f4.patient_id = f5.patient_id\n"
		        + "                        left join (select k.patient_id, k.visit_date\n"
		        + "                                   from kenyaemr_etl.etl_art_fast_track k) k\n"
		        + "                                  on f4.patient_id = k.patient_id\n"
		        + "               where f4.next_appointment_date between date(:startDate) and date(:endDate)\n"
		        + "                  or f4.refill_date between date(:startDate) and date(:endDate)\n"
		        + "               group by f5.patient_id\n"
		        + "               having next_appointment_date between date(:startDate) and date(:endDate)\n"
		        + "                   or next_refill_date between date(:startDate) and date(:endDate)\n"
		        + "               union\n"
		        + "               -- Never Returned\n"
		        + "               select f0.patient_id              as patient_id,\n"
		        + "                      f0.visit_date              as app_visit,\n"
		        + "                      f7.return_date             as return_date,\n"
		        + "                      k.latest_refill_visit_date as refill_return_date,\n"
		        + "                      f7.latest_appointment,\n"
		        + "                      f7.latest_refill_tca_date\n"
		        ...
		        + "                                          mid(max(concat(f7.visit_date, f7.refill_date)), 11)           as latest_refill_tca_date\n"
		        + "                                   from kenyaemr_etl.etl_patient_hiv_followup f7\n"
		        + "                                   group by f7.patient_id) f7\n"
		        + "                                  on f0.patient_id = f7.patient_id\n"
		        + "                        left join (select k.patient_id, max(k.visit_date) as latest_refill_visit_date\n"
		        + "                                   from kenyaemr_etl.etl_art_fast_track k\n"
		        + "                                   group by k.patient_id) k\n"
		        + "                                  on f0.patient_id = k.patient_id\n"
		        + "               where f0.next_appointment_date between date(:startDate) and date(:endDate)\n"
		        + "                  or f0.refill_date between date(:startDate) and date(:endDate)\n"
		        + "                   and (f7.return_date = f0.visit_date or k.latest_refill_visit_date = f0.visit_date) -- Check whether accurate\n"
		        + "               group by f7.patient_id, k.patient_id\n"
		        + "               having latest_appointment between date(:startDate) and date(:endDate)\n"
		        + "                   or latest_refill_tca_date between date(:startDate) and date(:endDate)) r\n"
		        ...
		        + "      where next_refill_date between date(:startDate) and date(:endDate)\n"
		        + "         or next_appointment_date between date(:startDate) and date(:endDate)\n"
