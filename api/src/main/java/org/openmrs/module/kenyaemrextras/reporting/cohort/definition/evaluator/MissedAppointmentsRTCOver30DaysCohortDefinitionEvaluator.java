...
public class MissedAppointmentsRTCOver30DaysCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		String qry = "select a.patient_id\n"
		        ...
		        + "                                             -- f6.refill_app_date    refill_app_date,\n"
		        ...
		        + "                                                                  max(k.visit_date)                                             as refill_app_date,\n"
		        ...
		        + "                                                                            left join kenyaemr_etl.etl_art_fast_track k on f5.patient_id = k.patient_id\n"
		        + "                                                           where f5.next_appointment_date between date(:startDate) and date(:endDate) or f5.refill_date between date(:startDate) and date(:endDate)\n"
		        ...
		        + "                                              f7.refill_return_date,\n"
		        + "                                      f7.latest_appointment,\n"
		        + "                                      f7.latest_refill_appointment\n"
		        ...
		        + "                                                          mid(max(concat(f7.visit_date, f7.refill_date)), 11) as latest_refill_appointment,\n"
		        ...
		        + "                               having latest_appointment between date(:startDate) and date(:endDate) or latest_refill_appointment between date(:startDate) and date(:endDate)\n"
		        + "                           ) r\n"
		        ...
		        + "                where (timestampdiff(DAY,next_appointment_date,return_date) > 30 and refill_return_date is null) or (timestampdiff(DAY,refill_tca,refill_return_date) > 30 and refill_return_date);";
